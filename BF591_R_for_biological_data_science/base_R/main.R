...
less_than_zero <- function(x) { return(x<0)
# main.R final product updated. This i
}
...
summarize_matrix <- function(x, na.rm=FALSE) {
  ...
  mean_col <- apply(x, 1, mean, na.rm = na.rm)
  stdev_col <- apply(x, 1, sd, na.rm = na.rm)
  median_col <- apply(x, 1, median, na.rm = na.rm)
  min_col <- apply(x, 1, min, na.rm = na.rm)
  max_col <- apply(x, 1, max, na.rm = na.rm)
  num_lt_0_col <- apply(x, 1, function(row) sum(row < 0, na.rm = na.rm))
  num_btw_1_and_5_col <- apply(x, 1, function(row) sum(is_between(row, 1, 5), na.rm = na.rm))
  num_na_col <- apply(x, 1, function(row) sum(is.na(row)))
  
  # Create a list with the calculated columns
  result_list <- list(
    mean = mean_col,
    stdev = stdev_col,
    median = median_col,
    min = min_col,
    max = max_col,
    num_lt_0 = num_lt_0_col,
    num_btw_1_and_5 = num_btw_1_and_5_col,
    num_na = num_na_col
  )
  
  # Convert the list to a data frame
  data_frame <- as.data.frame(result_list)
